"""
This is a preliminary data analysis file.
Step-by-step analysis of the historical data that is downloaded from the external sources 
(links provided in config.py) that combines:
- stadium info
- penalty cards given during each match
- match results with season, round, teams
- ELO numbers per team during each match. ELO numbers were preliminary obtained using ELO scraper.

Returns:
    group of CSV files: each file contains cleaned data per league.
"""
...
#Penalty cards during match - match_df
link=config.HISTORICAL_PENALTY_CARDS_LINK
#'https://aicore-files.s3.amazonaws.com/Data-Science/Match_Info.csv'
match_df = pd.read_csv(link)

#ELOs - ELO_df
#ELO_scraper.py is the scraper code that scrapes the ELO scores from the website.
pickle_link=config.HISTORICAL_ELO_FILE
myfile = pickle.load(urllib.request.urlopen(pickle_link))
ELO_df=(pd.DataFrame(myfile)).T

#Stadiums - stadium_df
link=config.STADIUM_DATA_LINK
...
# UPLOAD game results data per league
link = config.HISTORICAL_MATCH_RESULTS_FILE
...
for league in folder_list:
    ...
    os.makedirs('/home/irinakw/AiCore/FooballMatchPrediction/data/processed/clean_data_by_league', exist_ok=True)  
